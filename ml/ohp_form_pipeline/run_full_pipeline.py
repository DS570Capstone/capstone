"""
run_full_pipeline.py — End-to-end pipeline from existing processed outputs.

This script orchestrates the full workflow:
    1. Extract training data from the S&C manual PDF + existing video artifacts
    2. Train the ExerciseAware vision adapter
    3. LoRA fine-tune the language model
...
BATCH_OUTPUT_DIR = (
    r"C:\Users\josep\Desktop\Capstone_new\videos\First 500 Vids\batch_outputs"
)
...
def step_train(epochs: int = 10, batch_size: int = 4):
    ...
    log_path = os.path.join(
        CHECKPOINT_DIR, "logs", f"train_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    )
...
def main():
    ...
    parser.add_argument(
        "--step",
        choices=["all", "dataset", "train", "infer"],
        default="all",
    )
    parser.add_argument("--video", default=None, help="Video path for inference step")
    parser.add_argument(
        "--max_videos", type=int, default=0, help="Limit videos to process (0=all)"
    )
